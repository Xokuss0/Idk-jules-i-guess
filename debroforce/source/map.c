...
void map_init(Map* map) {
    for (int i = 0; i < MAP_WIDTH * MAP_HEIGHT; i++) {
        int y = i / MAP_WIDTH;
        // Ground at y=10 (160px), well within 240px screen height
        if (y > 10) map->tiles[i] = TILE_DIRT;
        else if (y == 10) map->tiles[i] = TILE_GRASS;
...
void map_draw(Map* map) {
    for (int y = 0; y < MAP_HEIGHT; y++) {
        for (int x = 0; x < MAP_WIDTH; x++) {
            ...
            else if (t == TILE_GRASS) c = 0x228B22FF; // Forest Green (Solid)
        ...
        for (int dx = -tr; dx <= tr; dx++) {
            if (dx*dx + dy*dy <= (tr+0.5)*(tr+0.5)) {
                ...
                if (t != TILE_EMPTY) {
                    ...
                    Color c = (t == TILE_DIRT) ? 0x8B4513FF : (t == TILE_GRASS ? 0x228B22FF : 0x808080FF);
